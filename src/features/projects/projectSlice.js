...
export const {
  addProject, updateProject, deleteProject, setSearch, setFilter
...
export const selectStats = createSelector(allProjects, list => {
  ...
  return {
    ...
    overdue:   list.filter(p =>
      p.dueDate && p.status !== 'Completed' && new Date(p.dueDate) < today
    ).length,
  }
})

export const selectById = id =>
  createSelector(allProjects, list => list.find(p => p.id === id))